...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 lg:px-20 h-20 flex items-center justify-between">
          {/* Logo */}
          <a
            href="#hero"
          ...
          <nav className="hidden lg:flex items-center gap-7">
            {navLinks.map((link) => (
              <a
                ...
                className="text-slate-600 hover:text-[#2563eb] text-sm font-medium transition-colors duration-200 relative group whitespace-nowrap"
              >
                {link.label}
            ...
            <nav className="flex flex-col gap-6 mt-8">
              {navLinks.map((link) => (
                <a
                  ...
                  href={link.href}
                  onClick={(e) => { e.preventDefault(); handleNav(link.href); }}
                  className="text-slate-800 text-xl font-semibold border-b border-slate-100 pb-4 hover:text-[#2563eb] transition-colors"
                >
                  {link.label}
                </a>
              ))}
              <button
                ...
                className="mt-4 px-6 py-4 text-white text-base font-semibold rounded-xl text-center"
